...
angular.module('home')
	...
	.controller('homPortalFavoriteCtrl', ['$scope', '$translate', 'resUser', 'qbTimelineService', function($scope, $translate, resUser, qbTimelineService) {
		...
		$scope.getFeedPosts = function() {
			...
			// fire
			qbTimelineService.getFavoriteFeedPost($scope.feed).then(function(more){
				if (more) {
					$scope.busy = false;
				}
			});
		...
		$scope.submitPost = function(text, photos) {
			return qbTimelineService.submitPost($scope.feed, text, photos);
		};
